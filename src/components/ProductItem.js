...
const ProductItem = ({product}) => {
  ...
  return (
    <Link to={`/product/${product?._id}`} className='w-[15rem] h-[11rem] mb-[10rem] relative'>
      <div className='w-[100%] flex justify-end'><AiOutlineHeart /></div>  
      <img className='w-[15rem] h-[15rem] object-cover' src={product.image} />
      
      <div className="flex items-center w-[100%] justify-between px-[.5rem]">
          <p className="font-bold text-[1rem] z-10">{product.name}</p>
